...
from composio_crewai import Actidd on, ComposioToolSet
...
# Trigger instance
composio_toolset1 = ComposioToolSet()
...
def callback_new_message(event: TriggerEventData) -> None:
    ...
    uid = user['uid']
    # Tools
    composio_toolset = ComposioToolSet(output_dir=Path.cwd()/"attachments",entity_id=event.metadata.connection.clientUniqueUserId)
    ...
    google_assistant = Agent(
        ...
        goal="Process emails, check for keywords, download attachments, extract attributes, and store in Google Sheets",
        backstory="You're an AI assistant that processes emails, extracts information from attachments, and stores data in Google Sheets.",
        verbose=True,
        llm=llm,
        tools=tools,
    ...
    process_new_email = Task(
        ...
        5. Store the identified attributes in Google Sheet with ID {sheetId}, sheet name {sheetName}, in cell A{row}. Do not use includeValuesInResponse attribute.
        ...
        expected_output="Summary of email processing, including whether keywords matched, attachment was processed, and data was stored in the Google Sheet.",
    )
